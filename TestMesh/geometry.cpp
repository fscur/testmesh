...
void geometry::addVertices(std::vector<vertex> &vertices, std::vector<uint> &indices, std::vector<glm::mat4> &modelMatrices)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, verticesSize, vertexBuffer, GL_DYNAMIC_DRAW);
    ...
    glBufferData(GL_ARRAY_BUFFER, normalsSize, normalBuffer, GL_DYNAMIC_DRAW);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, _indicesSize, &indices[0], GL_DYNAMIC_DRAW);
...
void geometry::change(std::vector<vertex> &vertices, std::vector<uint> &indices)
{
    _vertices = vertices;
    _indices = indices;
