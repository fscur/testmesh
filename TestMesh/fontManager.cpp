...
glyph* fontManager::getGlyph(font* glyphFont, const ulong& glyphChar)
{
    std::tuple<font*, ulong> key(glyphFont, glyphChar);

    if (_glyphCache.find(key) != _glyphCache.end())
        return _glyphCache[key];

    auto glyph = glyphFont->getGlyph(glyphChar);
    ...
    glyph->texPos = glm::vec2((float)x * r, (float)y * r);
    glyph->texSize = glm::vec2((float)w * r, (float)h * r);
